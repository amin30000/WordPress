...
<?php
	...
	public function _authenticate($user, $username, $password) {
		...
		if (isset($_POST['wfls-token-jwt']) && is_string($_POST['wfls-token-jwt'])) {
			...
			if (!$jwt) { //Possibly user-corrupted or expired JWT
				return new \WP_Error('wfls_twofactor_invalid', wp_kses(__('<strong>VALIDATION FAILED</strong>: The 2FA code could not be validated. Please try logging in again.', 'wordfence'), array('strong'=>array())));
			}
	...
	public function _admin_menu() {
		...
		if ($useSubmenu) {
			add_submenu_page('Wordfence', __('امنیت ورود به سیستم', 'wordfence'), __('امنیت ورود به سیستم', 'wordfence'), Controller_Permissions::CAP_ACTIVATE_2FA_SELF, 'WFLS', array($this, '_menu'));
		}
		else {
			add_menu_page(__('امنیت ورود به سیستم', 'wordfence'), __('امنیت ورود به سیستم', 'wordfence'), Controller_Permissions::CAP_ACTIVATE_2FA_SELF, 'WFLS', array($this, '_menu'), Model_Asset::img('menu.svg'));
		}
	}
