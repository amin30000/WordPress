				...
				this.ajax('wordfence_addTwoFactor', {
					...
					if (res.ok) {
						if (mode == 'authenticator') {
							...
							var message = __('کد زیر را با برنامه احراز هویت خود اسکن کنید تا این حساب را اضافه کنید. برخی از برنامه های احراز هویت نیز به شما اجازه می دهند به جای آن نسخه متنی را تایپ کنید.') + "<br><div id=\"wfTwoFactorQRCodeTable\"></div><br><strong>" + __('Key:') + "</strong> <input type=\"text\"" + (self.isSmallScreen ? "" : " size=\"45\"") + " value=\"" + res.base32Secret + "\" onclick=\"this.select();\" readonly>";
