...
var knownMenuFields = {
	...
	'access_level' : $.extend({}, baseField, {
		...
		display: function(menuItem) {
			...
			var displayValue = (menuItem.template_id === '') ? '< سفارشی >' : requiredCap;
			if (extraCap !== '') {
				if (menuItem.template_id === '') {
	...
	'colors' : $.extend({}, baseField, {
		...
		display: function(menuItem, displayValue, input, containerNode) {
			...
			return 'جای جای. شما هرگز نباید این را ببینید.';
		},
	...
	'is_always_open' : $.extend({}, baseField, {
		caption: 'این منو را گسترده نگه دارید',
		advanced : true,
		onlyForTopMenus: true,
		type: 'checkbox',
		standardCaption: false
	...
	switch(field_settings.type){
		...
		case 'access_editor':
			inputBox = $('<input type="text" class="ws_field_value" readonly="readonly">')
                .add('<input type="button" class="button ws_launch_access_editor" value="ویرایش...">');
			break;
		...
		case 'color_scheme_editor':
			inputBox = $('<span class="ws_color_scheme_display">Placeholder</span>')
				.add('<input type="button" class="button ws_open_color_editor" value="ویرایش...">');
			break;
...
function ameOnDomReady() {
	...
	$('#ws-ame-show-advanced-colors').on('click', function() {
		...
		$(this).text(showAdvancedColors ? 'مخفی سازی تنظیمات پیشرفته' : 'نمایش تنظیمات پیشرفته');
	});
	...
	menuEditorNode.on('click', '.ws_open_color_editor, .ws_color_scheme_display', function() {
		...
		setUpColorDialog(title ? ('رنگ ها: ' + formatMenuTitle(title)) : 'رنگ ها');
			...
			content: {
				text: function() {
					var indicator = $(this),
						...
						text = 'Additional permission settings are available. Click "ویرایش..." to change them.',
						heading = '';
