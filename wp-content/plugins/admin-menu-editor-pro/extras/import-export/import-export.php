...
class wsAmeImportExportFeature {
	...
	public function add_import_export_tabs($tabs) {
		$tabs['import'] = 'درون ریزی';
		$tabs['export'] = 'برون بری';
		return $tabs;
	}
	...
	public function display_import_tab() {
		...
		if ( $step === 1 ) {
			...
			<p>
				<label for="upload">یک فایل را درون ریزی انتخاب کنید. (<?php printf('maximum size: %s', $formattedSize); ?>
					):</label>
				<br>
			...
			submit_button('بعدی &rarr;', 'primary', 'submit', true, array('disabled' => 'disabled'));
			echo '</form>';
		} else if ( $step === 2 ) {
	...
	public function display_export_tab() {
		...
		$this->wp_menu_editor->display_settings_page_header();
		echo '<h2>انتخاب کنید چه چیزی میخواهید برون بری شود.</h2>';
		...
		submit_button('دانلود فایل', 'primary', 'submit', true);
